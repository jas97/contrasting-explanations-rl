...
from autorl4do.explanations.util import Trajectory


def get_pref_trajectories(policyA, policyB, env, max_traj_len, num_episodes=1000, threshold=0.7):
    ...
    if len(disagreement_states) > 0:
        ...
        scores = [get_traj_score(Q_A[i], Q_B[i], Q_A_B[i], Q_B_A[i], Q_A_s[i], Q_B_s[i], state_importance[i]) for i in range(len(disagreement_traj))]
        filtered_traj = [t for i, t in enumerate(disagreement_traj) if scores[i] > threshold]
        ...
        print('Number of preference traj pairs: {} max(scores) = {}'.format(len(filtered_traj), max(scores)))
...
def get_Q_vals(policyA, policyB, env, disagreement_states, disagreement_outcomes):
    ...
    scaler_B.fit([[min(Q_B + Q_B_A + Q_A_s)], [max(Q_B + Q_B_A + Q_A_s)]])

    Q_A = [scaler_A.transform([[q_a]]).item() for q_a in Q_A]
    Q_B = [scaler_B.transform([[q_b]]).item() for q_b in Q_B]

    Q_A_B = [scaler_A.transform([[q_a]]).item() for q_a in Q_A_B]
    Q_B_A = [scaler_B.transform([[q_b]]).item() for q_b in Q_B_A]

    Q_A_s = [scaler_A.transform([[q_s]]).item() for q_s in Q_A_s]
    Q_B_s = [scaler_B.transform([[q_s]]).item() for q_s in Q_B_s]
...
def get_traj_score(Q_A, Q_B, Q_A_B, Q_B_A, Q_A_s, Q_B_s, state_importance):
    ...
    traj_A = abs(Q_A - Q_A_s)
    traj_B = abs(Q_B - Q_B_s)

    return ((1 - abs(traj_B - traj_A)) + state_importance + traj_score)/ 4 # because importance is 2 things
